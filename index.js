 ...
 * Core Engine: Pure Stream Passthrough (No Header Injection), Semantic Routing, Live Debugger.
 */
    ...
    <body>
        <div class="container">
            <div class="header">
                ...
                <p class="subtitle">L'expérience IPTV centralisée, triée et universelle.</p>
            </div>
...
app.get('/:config/manifest.json', (req, res) => {
    ...
    if (config.search) {
        baseCatalogs = baseCatalogs.map(cat => {
            return { ...cat, extra: [{ name: 'search', isRequired: false }, { name: 'skip', isRequired: false }] };
        });
    }
...
app.get('/:config/stream/tv/:id.json', async (req, res) => {
    ...
    try {
            ...
            try {
                ...
                if (streamRes.data && streamRes.data.streams) {
                    return streamRes.data.streams.map((s, idx) => {
                        ...
                        // LÂCHER-PRISE SUR LES BEHAVIOR HINTS : On garde ce que TV Mio envoie sans ajouter de Referer bloquant
                        if (!outStream.behaviorHints) {
                            outStream.behaviorHints = {};
                        }
        ...
        setTimeout(() => streamCache.delete(cacheKey), 60000); 
