...
// Base de Données Logos VIP Complète & Élargie
const LOGOS = {
...
function parseConfig(encodedConfig) {
    try {
        ...
        parsed.logos = false; // Bloqué sur Off comme demandé
        return parsed;
    } catch (e) {
...
function extractMatchEvent(rawName) {
    ...
    // Gomme tout ce qui est entre () ou [] pour éviter de confondre les dates ou les FPS avec des scores
    s = s.replace(/\([^)]*\)/g, '').replace(/\[[^\]]*\]/g, '').trim();

    let isMatch = false;
    let eventName = '';

    // Détection MATCH TIME (Force l'isolement)
...
// ============================================================================
// 2. MOTEUR SÉMANTIQUE D'AIGUILLAGE DE CHAINES FIXES
// ============================================================================
function getChannelData(rawName) {
    ...
    // Nettoyage radical (On efface ce qui est entre parenthèses ou crochets)
    ...
    if (n.includes('LIGUE 1') || n.includes('LIGUE1') || n.match(/\bL1\b/)) {
        if (!n.includes('DAZN') && !n.includes('BEIN') && !n.includes('RMC')) {
            // Extraction robuste du numéro (LIGUE 1 PLUS 3)
            let m = n.match(/(?:LIGUE\s*1|L1|LIGUE1).*?PLUS.*?(\d+)/i);
            let num = m ? m[1] : '1';
            if (num === '1') {
    ...
    // --- DAZN ---
    if (n.includes('DAZN')) {
        if (n.includes('RISE')) return { id: 'hyb_sport_dazn_rise', name: 'DAZN Rise', categories: ['sports'], index: 150 }; // DAZN Rise relégué
    ...
    if (c.includes('CANAL') || c.includes('CPLUS')) {
        if (c.includes('LIGA') || c === 'CANALPLUSL') return null; // Exclusion Canal+ L
        ...
        // Canal+ Live relégué en backup Canal (non sport)
        if (c.includes('LIVE')) {
    ...
    // --- AUTRES SPORTS (Eurosport AVANT RMC) ---
    ...
    if (c.includes('GULLI')) return { id: 'hyb_jeu_gulli', name: 'Gulli', categories: ['jeunesse', 'tnt'], index: 18 }; // Gulli à sa place TNT
    ...
    if (c.includes('MANGAS')) return { id: 'hyb_jeu_mangas', name: 'Mangas', categories: ['jeunesse'], index: 9 };
    if (c.includes('GAMEONE') || c.match(/\bG1\b/) || c === 'G1') return { id: 'hyb_jeu_gameone', name: 'Game One', categories: ['jeunesse'], index: 10 };
...
async function fetchCatalogFromSource(sourceInput) {
    ...
    try {
        ...
        const catalogPromises = catalogs.map(async (catalog) => {
            ...
            // 🔴 L'ASPIRATEUR EST DÉBRIDÉ : Scanne jusqu'à 30 pages (3000 flux) par dossier du fournisseur IPTV
...
async function getChannelsForSources(sourcesList) {
    ...
    for (let i = 0; i < sourcesList.length; i++) {
        ...
        metas.forEach(meta => {
            ...
            // On conserve toujours les logos du M3U par défaut, ou on injecte nos logos VIP (transparents)
...
app.get('/:config/stream/tv/:id.json', async (req, res) => {
    ...
    // SYSTÈME DE CACHE : Anti-Timeout Stremio
    ...
    try {
        let streamPromises = channel.sources.map(async (source) => {
            ...
            try {
                // Timeout sécurisé pour que Stremio n'abandonne pas
                ...
                if (streamRes.data && streamRes.data.streams) {
                    return streamRes.data.streams.map((s, idx) => {
                        ...
                        // Nettoyage radical des parenthèses/crochets pour analyser l'ADN sans polluer le tri
        ...
        // AFFICHAGE : Nom du flux original en GROS, qualité en dessous
        ...
        setTimeout(() => streamCache.delete(cacheKey), 300000); // Nettoyage cache après 5 minutes
