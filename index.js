 ...
 * Core Engine: IP Spoofing (Anti-403), 4s Ping Scanner, 15s Micro-Cache, Absolute Routing.
 */
...
// --- STRICT SEMANTIC ROUTING (VERSION 8.2.0 BASE) ---
function getChannelData(rawName) {
    ...
    // Les VIP du Sport (Ligue 1 & DAZN) tout en haut !
    ...
    <body>
        <div class="container">
            <div class="header">
                ...
                <p class="subtitle">L'expérience IPTV centralisée, triée et universelle (v1.0.1).</p>
            </div>
...
app.get('/:config/manifest.json', (req, res) => {
    ...
    res.json({
        ...
        version: '1.0.1',
        name: 'HybridTV',
        description: 'Meta-Addon IPTV (v1.0.1). IP Spoofing, 4s Ping Scanner, 15s Micro-Cache.',
...
app.get('/:config/stream/tv/:id.json', async (req, res) => {
    ...
    // NOUVEAUTÉ : On récupère l'IP du client Stremio (Ta télé/Ton PC) pour l'envoyer à TV Mio
    ...
    // MICRO-CACHE 15 SECONDES pour optimiser le rafraîchissement des jetons
    ...
    try {
        let streamPromises = channel.sources.map(async (source) => {
            ...
            try {
                ...
                // IP SPOOFING : On fait croire à TV Mio que c'est ta télé qui demande le lien, pas le serveur Render !
        ...
        // Cache optimisé à 15 secondes
        streamCache.set(cacheKey, finalStreams);
        setTimeout(() => streamCache.delete(cacheKey), 15000); 
